let elem = document.getElementById( '__debug_kit' );
if( elem ) {
...
( function( win, doc ) {
    ...
    let onReady = function() {
        ...
        iframe.height = 40;
        iframe.width = 40;
        iframe.src = win.__debugKitBaseUrl + 'debug-kit/toolbar/' + win.__debugKitId;
    ...
    let logAjaxRequest = function( original ) {
        return function() {
            if( this.readyState === 4 && this.getResponseHeader( 'X-DEBUGKIT-ID' ) ) {
                ...
                iframe.contentWindow
                    .postMessage( 'ajax-completed$$' + JSON.stringify( params ), window.location.origin );
            }
            if( original ) {
